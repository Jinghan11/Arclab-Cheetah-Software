...
void MiniCheetahHardwareBridge::run() {
  ...
  // imu
  if(_imuInit)
    _imuThread = std::thread(&MiniCheetahHardwareBridge::run_imu, this);
...
void MiniCheetahHardwareBridge::run_imu() {
  while(true) {
    _N100IMU.run();

#ifdef USE_N100
    _vectorNavData.accelerometer = _N100IMU.acc;    //acceleration
    _vectorNavData.quat[0] = _N100IMU.quat[1];
    _vectorNavData.quat[1] = _N100IMU.quat[2];
    _vectorNavData.quat[2] = _N100IMU.quat[3];
    _vectorNavData.quat[3] = _N100IMU.quat[0];      
    _vectorNavData.gyro    = _N100IMU.gyro;         //angula velocity
#endif
  }
...
void MiniCheetahHardwareBridge::logimu() {
  _N100IMU.updateLCM(&_imuData);
  _imuLcm.publish("imu", &_imuData);
}
